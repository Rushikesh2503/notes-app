...
const appDirectoryName = "NotesApp";
...
const getRootDir = () => {
  return `${os.homedir()}/${appDirectoryName}`;
};
const getNotes = async () => {
  ...
  if (lodashExports.isEmpty(notes)) {
    ...
    const content = await fsExtra.readFile(welcomeNoteFile, { encoding: fileEncoding });
    await fsExtra.writeFile(`${rootDir}/${welcomeNoteFilename}`, content, { encoding: fileEncoding });
...
const readNote = async (filename) => {
  const rootDir = getRootDir();
  return fsExtra.readFile(`${rootDir}/${filename}.md`, { encoding: fileEncoding });
};
const writeNote = async (filename, content) => {
  const rootDir = getRootDir();
  return fsExtra.writeFile(`${rootDir}/${filename}.md`, content, { encoding: fileEncoding });
};
